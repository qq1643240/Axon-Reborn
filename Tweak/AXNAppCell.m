...
-(void)handleSwipeUp:(UISwipeGestureRecognizer *)sender {
    if (!self.swipeClearEnabled) return;
    if (sender.state == UIGestureRecognizerStateEnded) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            self.transform = CGAffineTransformMakeTranslation(0, -self.frame.size.height * 1.5);
            self.alpha = 0.0;
...
-(void)handleSwipeDown:(UISwipeGestureRecognizer *)sender {
    if (!self.swipeClearAllEnabled) return;
    if (sender.state == UIGestureRecognizerStateEnded) {
        // 触觉反馈
        AudioServicesPlaySystemSound(1520); // 更强的反馈表示全部清除
